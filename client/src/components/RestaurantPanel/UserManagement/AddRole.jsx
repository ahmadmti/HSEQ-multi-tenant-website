...
export default function Form() {
    ...
    const getItems = () => { dispatch(toggle());
        getMenuItems()
            .then(res => { dispatch(toggle());
                                        ...
                                        <div className="text-center">
                                            <Controller
                                                ...
                                                as={<TextField id="role_name" margin="dense" variant="outlined"
                                                    ...
                                                    style={{ width: '70%' }} label={t("Role_name")}
                                    ...
                                    <Grid item md={12} sm={12} xs={12}  >
                                        <div className="text-center">
                                            <FormControl error={errors.permissions ? true : false} variant="outlined" style={{ width: '70%' }} className={classes.formControl}>
                                                <InputLabel id="demo-mutiple-chip-label">{t("Permission")}</InputLabel>
                                                <Controller
                                                    name="permissions"
                                                    as={<Select
                                                            ...
                                                            return (item.children && item.children.length > 0) ?
                                                                ...
                                                                item.children.map((child, i) => {
                                                                    return <MenuItem key={i + 'ii'} value={child}>
                                                                       {t(`${camelize(child.name.toLowerCase())}`)}
                                                                    </MenuItem>
                                                                })
